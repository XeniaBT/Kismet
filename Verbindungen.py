...
#            detect_id=input("Geben Sie ein ID der Detection ein: ")
            ...
            output.write("ClientMAC,ClientBSSID,ClientType1,ClientType2,NW_Type, Manuf,Channel,f_y,f_m,f_d,l_y,l_m,l_d,f_h,f_min,f_s,l_h,l_min,l_s,data,crypt,total,ClientPower,ClientESSID,Privacy,Cipher,Auth,Lat,Lon,ClientSSIDs,DetectionID\n")
...
#   ClientType: wenn type=tods und SSID existiert => es ist ein Client, der nach AP sucht           
            ...
            ssid = client.getiterator('SSID')
            cSSIDliste3=""
            clientSSID="" 
            clientSSIDlist=list()
            if ssid is not None:
                for s in ssid:
                    if s.find('ssid') is not None:
                        clientSSID = s.find('ssid').text
                    clientSSIDlist.append(clientSSID)
                    
          #  clientSSIDlist.append(clientSSID)
                cSSIDliste3 = str(' '.join(str(x) for x in clientSSIDlist))           
            #cSSIDliste3 = str(clientSSIDlist) 
 ...
 #                   continue  
            ...
            if power is not None:
                ...
                c = client_mac, bssid, ClientType1, ClientType2, NWtype, manuf, channel, f_y, f_m, f_d, l_y, l_m, l_d, f_h, f_min, f_s, l_h, l_min, l_s, data, crypt, total, client_power, essid_text, privacy, cipher, auth, lat, lon, cSSIDliste3
